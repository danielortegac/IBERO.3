...
var MICROSOFT_CLIENT_SECRET = process.env.MICROSOFT_CLIENT_SECRET;
var msalConfig = {
  auth: {
...
if (MICROSOFT_CLIENT_ID && MICROSOFT_CLIENT_SECRET) {
  ...
  console.warn("ADVERTENCIA: MICROSOFT_CLIENT_ID o MICROSOFT_CLIENT_SECRET no est\xE1n configurados. La integraci\xF3n con Microsoft no funcionar\xE1.");
}
var adminOptions = {};
...
async function requireFirebaseUser(req, res, next) {
  ...
  try {
    ...
    return res.status(401).json({ ok: false, error: "Token de autenticaci\xF3n inv\xE1lido o expirado." });
  }
}
var ENCRYPTION_KEY = process.env.EMAIL_ENCRYPTION_KEY ? cryptoNode.scryptSync(process.env.EMAIL_ENCRYPTION_KEY, "salt", 32) : null;
var ALGORITHM = "aes-256-gcm";
if (!ENCRYPTION_KEY) {
  console.warn("ADVERTENCIA: EMAIL_ENCRYPTION_KEY no est\xE1 configurada. El cifrado de cuentas de correo no funcionar\xE1.");
}
var encryptAccountData = (data) => {
  if (!ENCRYPTION_KEY) {
    throw new Error("EMAIL_ENCRYPTION_KEY no est\xE1 configurada en el servidor.");
  }
...
var decryptAccountData = (encryptedData) => {
...
var syncAccountsToFirestore = async (userId, accounts) => {
  if (!userId) return;
  try {
...
async function startServer() {
  ...
  app.get("/api/debug/firestore-admin", async (req, res) => {
  ...
  const scheduledEmails = /* @__PURE__ */ new Map();
  const vapidKeys = {
  ...
  if (vapidKeys.publicKey && vapidKeys.privateKey) {
    ...
    console.warn("ADVERTENCIA: VAPID_PUBLIC_KEY o VAPID_PRIVATE_KEY no est\xE1n configurados. Las notificaciones Push no funcionar\xE1n.");
  }
    ...
    snapshot.docChanges().forEach(async (change) => {
      if (change.type === "added") {
        ...
        if (pathParts.length >= 4) {
          ...
          if (Date.now() - timestamp > 6e4) return;
          try {
            const pushSnap = await firestore.collection(`users/${userId}/push_subscriptions`).get();
            if (!pushSnap.empty) {
              const payload = JSON.stringify({
                title: notif.type === "new_message" ? "Nuevo Mensaje" : notif.type === "incoming_call" ? "Llamada Entrante" : "Notificaci\xF3n Goatify",
                body: notif.text.replace(/<\/?[^>]+(>|$)/g, ""),
                // Limpiar HTML
                icon: "/Logos HD.png",
                data: { url: notif.link || "/" }
              });
  ...
  const reminderWorkerIntervalMs = Number(process.env.REMINDER_WORKER_INTERVAL_MS || 6e4);
  const reminderLookaheadMinutes = Number(process.env.REMINDER_LOOKAHEAD_MINUTES || 90);
  const createReminderNotification = async (userId, reminderId, payload) => {
  ...
  const processScheduledReminders = async () => {
    ...
    const ahead = new Date(now.getTime() + reminderLookaheadMinutes * 6e4);
    const nowIso = now.toISOString().slice(0, 19);
    const aheadIso = ahead.toISOString().slice(0, 19);
    try {
      const socialSnap = await firestore.collectionGroup("socialCalendar").where("scheduledAt", ">=", nowIso).where("scheduledAt", "<=", aheadIso).get();
      for (const docSnap of socialSnap.docs) {
        ...
        const reminderAt = new Date(scheduledDate.getTime() - minutesBefore * 6e4);
        if (now.getTime() < reminderAt.getTime() || now.getTime() > scheduledDate.getTime() + 5 * 6e4) continue;
        ...
        await createReminderNotification(userId, `social_${docSnap.id}_${minutesBefore}`, {
          ...
          text: `\u{1F4E3} ${platform}: ${title}. Toca para abrir tu calendario de contenidos.`,
          link: "/#/calendar"
        });
      }
    } catch (e) {
    ...
    try {
      const callsSnap = await firestore.collection("calls").where("scheduledAt", ">=", nowIso).where("scheduledAt", "<=", aheadIso).get();
      for (const docSnap of callsSnap.docs) {
        ...
        const scheduledDate = new Date(String(scheduledAt));
        if (Number.isNaN(scheduledDate.getTime())) continue;
        const reminderAt = new Date(scheduledDate.getTime() - 30 * 6e4);
        if (now.getTime() < reminderAt.getTime() || now.getTime() > scheduledDate.getTime() + 5 * 6e4) continue;
        ...
        for (const uid of recipients) {
          await createReminderNotification(String(uid), `meet_${docSnap.id}_30_${uid}`, {
            ...
            title: "Reuni\xF3n pr\xF3xima",
            text: `\u{1F3A5} ${call.title || "Reuni\xF3n Goatify"} empieza pronto.`,
  ...
  if (reminderWorkerEnabled) {
    ...
    setTimeout(processScheduledReminders, 15e3);
  }
  if (process.env.ENABLE_MAIL_POLLING === "true") {
    const mailPollIntervalMs = Number(process.env.MAIL_POLL_INTERVAL_MS || 9e5);
    setInterval(async () => {
      try {
    ...
    console.log("[MAIL POLLING] Desactivado. Activa ENABLE_MAIL_POLLING=true si necesitas monitoreo IMAP autom\xE1tico.");
  }
  setInterval(async () => {
    const now = Date.now();
    for (const [jobId, job] of scheduledEmails.entries()) {
      if (now >= job.sendAt) {
        ...
        if (ownerSession && ownerSession.accounts && ownerSession.accounts.length > 0) {
          ...
          await refreshAccountTokens(acc);
          const transporterOptions = getTransporterOptions(acc);
          try {
    ...
    try {
      const queueSnap = await firestore.collection("mail_queue").where("status", "==", "pending").limit(10).get();
      if (!queueSnap.empty) {
        for (const doc of queueSnap.docs) {
          ...
          if (!sessionToUse && job.isSystemNotification) {
            for (const sess of sessions.values()) {
              if (sess.accounts && sess.accounts.some((a) => a.email === "info@goatify.app")) {
                sessionToUse = sess;
                break;
              }
            }
          ...
          if (!dailyCheck.exists) {
            ...
            if (!newsSnap.empty) {
              ...
              if (articles.length > 0) {
                const slugify = (text) => {
                  return text.toString().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim().replace(/\s+/g, "-").replace(/[^\w-]+/g, "").replace(/--+/g, "-").replace(/^-+/, "").replace(/-+$/, "");
                };
                const newsItemsHtml = articles.slice(0, 6).map((n, i) => {
                  ...
                  return `
                            <div style="margin-bottom: 30px; padding: 25px; background-color: #ffffff; border-radius: 16px; border: 1px solid #e2e8f0; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
                                ...
                                <div style="background-color: #f1f5f9; padding: 15px 18px; border-radius: 12px; margin-bottom: 20px;">
                                    <p style="margin: 0; font-size: 13px; color: #1e293b; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 5px;">\u{1F9EC} Consejo para tu Marca:</p>
                                ...
                                <div style="text-align: right;">
                                    <a href="${articleLink}" style="display: inline-block; color: #10b981; font-size: 14px; font-weight: 800; text-decoration: none; text-transform: uppercase; letter-spacing: 0.5px;">Leer an\xE1lisis completo &rarr;</a>
                                </div>
                            </div>`;
                }).join("");
...
<body style="margin: 0; padding: 40px 10px; background-color: #f8fafc;">
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; width: 100%; max-width: 680px; margin: 0 auto;">
        <div style="background-color: #0f172a; padding: 45px 30px; text-align: center; border-radius: 24px 24px 0 0; border-bottom: 5px solid #10b981;">
            <p style="color: #10b981; margin: 0 0 10px 0; font-size: 12px; font-weight: 900; text-transform: uppercase; letter-spacing: 3px;">PULSO DE INNOVACI\xD3N</p>
            <h1 style="color: #ffffff; margin: 0; font-size: 32px; font-weight: 900; letter-spacing: -1px; line-height: 1;">NOTICIAS DEL D\xCDA</h1>
            <p style="color: #94a3b8; margin: 15px 0 0 0; font-size: 14px; font-weight: 500; line-height: 1.4;">Consejos estrat\xE9gicos y tendencias para potenciar tu marca con Inteligencia Artificial.</p>
        </div>
        ...
        <div style="background-color: #ffffff; padding: 35px 30px; border-radius: 24px; border: 1px solid #e2e8f0; text-align: center; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.05); margin-bottom: 30px;">
            <div style="width: 60px; height: 60px; background-color: #ecfdf5; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center; margin-bottom: 20px;">
                <span style="font-size: 30px;">\u{1F911}</span>
            </div>
            <h2 style="margin: 0 0 10px 0; font-size: 22px; font-weight: 900; color: #0f172a; letter-spacing: -0.5px;">\xBFQuieres generar ingresos extra?</h2>
            <p style="margin: 0 0 25px 0; font-size: 15px; color: #64748b; line-height: 1.6;">\xDAnete a nuestro programa de socios y genera comisiones recurrentes vendiendo nuestras soluciones tecnol\xF3gicas de \xE9lite.</p>
        ...
        <div style="text-align: center; padding: 20px;">
            ...
            <p style="margin: 0; font-size: 11px; font-weight: 600; color: #94a3b8; text-transform: uppercase; letter-spacing: 1px;">\xA9 2026 Inteligencia que Transforma Negocios.</p>
        </div>
    </div>
</body>
</html>`;
                ...
                await firestore.collection("mail_queue").doc(todayId).set({
                  ...
                  to: "",
                  bcc: bccList,
                  subject: "Noticias del d\xEDa y consejos para tu marca",
  ...
  if (!GOOGLE_CLIENT_ID || !GOOGLE_CLIENT_SECRET) {
    console.warn("ADVERTENCIA: GOOGLE_CLIENT_ID o GOOGLE_CLIENT_SECRET no est\xE1n configurados. La integraci\xF3n con Google no funcionar\xE1 correctamente.");
  }
  ...
  const requireAuth = async (req, res, next) => {
    ...
    if (String(token).startsWith("goatify_")) {
      if (!sessions.has(token)) return res.status(401).json({ error: "Sesi\xF3n expirada. Vuelve a iniciar sesi\xF3n." });
    ...
    try {
      ...
      return res.status(401).json({ error: "Token inv\xE1lido o sesi\xF3n expirada." });
    }
  };
  const getImapClientOptions = (acc) => {
    ...
    const options = {
      ...
      port: 993,
      secure: true,
      auth: isGoogleOAuth ? { user: acc.email, accessToken: acc.tokens.access_token } : isMicrosoftOAuth ? { user: acc.email, accessToken: acc.tokens.access_token } : { user: acc.email, pass: acc.password },
      logger: false
    };
  ...
  const getTransporterOptions = (acc) => {
    const isGoogleOAuth = acc.provider === "google";
    const isMicrosoftOAuth = acc.provider === "microsoft";
    const isGmailManual = acc.provider === "gmail_manual";
    const isOutlookManual = acc.provider === "outlook_manual";
  ...
  const refreshAccountTokens = async (acc) => {
  ...
  function getValidGeminiApiKey() {
    ...
    const ignored = [];
    let selected = { key: null, source: null };
    for (const k of keys) {
      if (!k.val) continue;
  ...
  const ALLOWED_TEXT_FLASH_MODELS = /* @__PURE__ */ new Set([
  ...
  const ALLOWED_TEXT_PRO_MODELS = /* @__PURE__ */ new Set([
    "gemini-2.5-pro",
    "gemini-3.1-pro-preview"
  ]);
  const normalizeTextModelName = (model) => {
    return String(model || "").replace(/^models\//, "").trim();
  };
  const modelLooksPro = (model) => /(^|[-/])pro($|-)/i.test(model);
  async function getUserAiPlan(uid) {
    try {
  ...
  function fallbackTextModelForPlan(plan, moduleName) {
    if (plan === "free" || moduleName === "chat") return TEXT_LITE_MODEL;
    return TEXT_FLASH_MODEL;
  }
  function tokenCapForPlan(plan, paidPremium, moduleName, requested) {
  ...
  async function resolveTextAiPolicy(req, requestedModel, moduleName) {
  ...
  const SERVER_FEATURE_LIMIT_MAP = {
  ...
  const serverDefaultCounters = (nowIso) => ({
    daily_chat_count: 0,
    daily_entry_count: 0,
    daily_form_edits: 0,
  ...
  function getEffectiveUserTimezone(userData = {}) {
  ...
  async function refreshUsageRollover(uid, userData = {}, usageRef = firestore.collection("user_usage").doc(uid)) {
    ...
    await firestore.runTransaction(async (tx) => {
      ...
      const updates = { "counters.last_activity": counters.last_activity || nowIso };
  ...
  function estimateAiCostUsd(model = "default", usageMetadata = {}, fallbackCost = 0) {
    ...
    const cost = tokensIn / 1e6 * inRate + tokensOut / 1e6 * outRate;
    return Number((cost > 0 ? cost : fallbackCost).toFixed(8));
  }
  async function recordServerUsageTelemetry(uid, payload) {
    if (!uid) return;
    const nowIso = (/* @__PURE__ */ new Date()).toISOString();
  ...
  async function consumeFeatureOrReject(req, featureKey, amount = 1, metadata = {}) {
    ...
    await firestore.runTransaction(async (tx) => {
      ...
      const userEmail = String(userData.email || req.user?.email || "").toLowerCase();
      const isSuperAdminUser = ["deoc29@gmail.com", "deoc29@hotmail.com", "vaoc93@hotmail.com", "info@goatify.app"].includes(userEmail) || Boolean(userData.isSuperAdmin);
      ...
      const rawUsage = usageSnap.exists ? usageSnap.data() || {} : defaultUsage;
      const counters = { ...serverDefaultCounters(nowIso), ...rawUsage.counters || {} };
      const updates = {
        plan_id: plan,
        [`counters.last_activity`]: nowIso
      };
      ...
      if (featureKey === "agent_response") limitValue += extraAgents * 1e3;
      ...
      if (featureKey === "storage") {
        ...
        if (limitInBytes > 0 && limitValue !== 999999 && currentValue + safeAmount > limitInBytes) {
          const err = new Error(`Espacio insuficiente. Tu plan permite ${limitValue} GB de Drive.`);
          err.status = 402;
          throw err;
        }
      } else if (limitValue !== 999999 && currentValue + safeAmount > limitValue) {
        const err = new Error(`L\xEDmite alcanzado para ${featureKey}. Usado: ${currentValue}/${limitValue}.`);
        err.status = 402;
        throw err;
      }
  ...
  async function releaseFeatureConsumption(req, featureKey, amount = 1) {
    const uid = req.user?.uid;
    if (!uid) return;
    const feature = SERVER_FEATURE_LIMIT_MAP[featureKey];
    const safeAmount = ["voice_live_minute", "video_live_minute"].includes(featureKey) ? Math.max(0.1, Number(amount) || 0.1) : Math.max(1, Number(amount) || 1);
  ...
  function sendLimitError(res, e) {
    const status = Number(e?.status || 402);
    return res.status(status).json({ ok: false, error: e?.message || "L\xEDmite del plan alcanzado.", code: "PLAN_LIMIT_REACHED" });
  }
  const PUBLIC_CONSUMABLE_FEATURES = Object.keys(SERVER_FEATURE_LIMIT_MAP);
  app.post("/api/usage/consume", requireFirebaseUser, async (req, res) => {
  ...
  app.post("/api/usage/release", requireFirebaseUser, async (req, res) => {
  ...
  app.get("/api/usage/current", requireFirebaseUser, async (req, res) => {
    try {
      const uid = req.user.uid;
      const now = /* @__PURE__ */ new Date();
      const nowIso = now.toISOString();
  ...
  async function recalculateUsageForUid(uid) {
    const nowIso = (/* @__PURE__ */ new Date()).toISOString();
    ...
    projectsSnap.forEach((pDoc) => {
      ...
      (p.documents || []).forEach((d) => {
        totalBytes += Number(d.size || d.sizeBytes || 0);
      });
      (p.clients || []).forEach((c) => (c.files || []).forEach((f) => {
        totalBytes += Number(f.size || f.sizeBytes || 0);
      }));
    });
    ...
    const activeSites = sitesSnap.docs.filter((d) => d.data()?.active !== false).length;
    recordingsSnap.forEach((d) => {
      totalBytes += Number(d.data()?.sizeBytes || d.data()?.size || 0);
    });
    stickersSnap.forEach(() => {
      totalBytes += 204800;
    });
    const driveData = driveSnap.exists ? driveSnap.data() || {} : {};
    if (Array.isArray(driveData.personalFiles)) {
      driveData.personalFiles.forEach((f) => {
        totalBytes += Number(f.size || f.sizeBytes || 0);
      });
    }
  ...
  app.post("/api/usage/sync", requireFirebaseUser, async (req, res) => {
    const uid = req.user.uid;
    const userSnap = await firestore.collection("users").doc(uid).get();
  ...
  app.post("/api/usage/recalculate", requireFirebaseUser, async (req, res) => {
    const stats = await recalculateUsageForUid(req.user.uid);
    return res.json({ ok: true, stats });
  });
  app.post("/api/usage/entry", requireFirebaseUser, async (req, res) => {
    const uid = req.user.uid;
    const usageRef = firestore.collection("user_usage").doc(uid);
    await firestore.runTransaction(async (tx) => {
      const snap = await tx.get(usageRef);
      ...
      const counters = data.counters || {};
      const lastDailyKey = counters.last_daily_reset ? getLocalDateKey(new Date(counters.last_daily_reset), userTimezone) : "";
      const updates = { "counters.last_entry_date": today, "counters.last_activity": nowIso };
  ...
  const OWNER_AGENT_FEATURES = ["ai_chat", "agent_response", "voice_live_minute", "video_live_minute"];
  async function canOwnerUseFeature(ownerId, featureKey, amount = 1) {
    const feature = SERVER_FEATURE_LIMIT_MAP[featureKey];
    ...
    if (subscriptionStatus === "canceled" && plan !== "free") plan = "free";
    if (!SERVER_PLAN_LIMITS[plan]) plan = "free";
    const counters = { ...serverDefaultCounters((/* @__PURE__ */ new Date()).toISOString()), ...(usageSnap.data() || {}).counters || {} };
    let limitValue = SERVER_PLAN_LIMITS[plan][feature.limitKey];
    if (typeof limitValue !== "number") limitValue = 0;
    const extraAgents = Number(userData.extraAgentsPurchased || 0);
    if (featureKey === "ai_chat") limitValue += extraAgents * 50;
  ...
  app.post("/api/usage/can-use-agent-owner", requireFirebaseUser, async (req, res) => {
    const { ownerId, agentId, featureKey, amount = 1 } = req.body || {};
    if (!ownerId || !agentId || !OWNER_AGENT_FEATURES.includes(featureKey)) {
      return res.status(400).json({ ok: false, error: "Solicitud de validaci\xF3n de agente inv\xE1lida." });
    }
  ...
  app.post("/api/usage/consume-agent-owner", requireFirebaseUser, async (req, res) => {
    const { ownerId, agentId, featureKey, amount = 1, metadata = {} } = req.body || {};
    if (!ownerId || !agentId || !OWNER_AGENT_FEATURES.includes(featureKey)) {
      return res.status(400).json({ ok: false, error: "Solicitud de consumo de agente inv\xE1lida." });
    }
    const agentSnap = await firestore.collection("agents").doc(agentId).get();
    if (!agentSnap.exists || agentSnap.data()?.ownerId !== ownerId) {
      return res.status(403).json({ ok: false, error: "Agente no autorizado para este owner." });
  ...
  app.post("/api/usage/release-agent-owner", requireFirebaseUser, async (req, res) => {
    const { ownerId, agentId, featureKey, amount = 1 } = req.body || {};
    if (!ownerId || !agentId || !OWNER_AGENT_FEATURES.includes(featureKey)) {
      return res.status(400).json({ ok: false, error: "Solicitud de rollback de agente inv\xE1lida." });
    }
    const agentSnap = await firestore.collection("agents").doc(agentId).get();
    if (!agentSnap.exists || agentSnap.data()?.ownerId !== ownerId) {
      return res.status(403).json({ ok: false, error: "Agente no autorizado para este owner." });
  ...
  app.get("/api/health/gemini", async (req, res) => {
    ...
    if (!apiKey) {
      report.status = "FAIL";
      report.details = "No se encontr\xF3 ninguna API Key v\xE1lida (debe empezar con AIza y tener >35 caracteres). Variables revisadas: GEMINI_API_KEY, API_KEY, GOOGLE_API_KEY.";
    ...
    try {
      ...
      if (result && result.text) {
        ...
        report.details = "Respuesta recibida pero estructura inesperada o texto vac\xEDo.";
        return res.json(report);
      }
    } catch (error) {
      report.status = "FAIL";
      ...
      if (message.includes("403") || message.includes("PERMISSION_DENIED") || message.includes("API key not valid")) {
        ...
        report.details = `404: El modelo ${targetModel} no est\xE1 disponible o no existe. Mensaje: ${message.substring(0, 50)}`;
  ...
  app.post("/api/perplexity", requireFirebaseUser, async (req, res) => {
    ...
    if (!PERPLEXITY_API_KEY) {
      ...
      return res.status(500).json({ error: "PERPLEXITY_API_KEY no est\xE1 configurada en el servidor." });
    }
    try {
      const response = await fetch(PERPLEXITY_API_URL, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "Se preciso, ejecutivo y proporciona informaci\xF3n actualizada. Responde siempre en el idioma en que se te pregunta."
            },
            {
              role: "user",
              content: query
      ...
      if (!response.ok) {
        ...
        if (creditConsumed) await releaseFeatureConsumption(req, "ai_grounding", 1);
        return res.status(response.status).json({ error: "Error en la b\xFAsqueda web" });
      }
      const data = await response.json();
      await recordServerUsageTelemetry(req.user.uid, { module: "search", model: "perplexity-sonar", usageMetadata: data.usage || {}, isPerplexity: true, fallbackCost: 5e-3, action: "perplexity_search" });
      res.json({
  ...
  app.post("/api/push/subscribe", requireAuth, async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ error: "No se pudo guardar la suscripci\xF3n" });
    }
  });
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      if (requestedUserId && requestedUserId !== decoded.uid) {
        return res.status(403).json({ error: "No puedes crear sesi\xF3n para otro usuario." });
      }
      ...
      res.status(401).json({ error: "Token inv\xE1lido o sesi\xF3n expirada." });
    }
  });
  app.post("/api/admin/deleteUser", requireFirebaseUser, async (req, res) => {
  ...
  app.post("/api/auth/identify", express.json(), async (req, res) => {
    const { token, userId } = req.query;
    const { accounts, isSuperAdmin } = req.body || {};
    if (token && userId) {
      if (!sessions.has(token)) {
        return res.status(401).json({ error: "Sesi\xF3n no inicializada. Vuelve a entrar a Mail." });
      }
      const sess = sessions.get(token);
      if (sess.userId && sess.userId !== userId) {
        return res.status(403).json({ error: "Sesi\xF3n no corresponde al usuario actual." });
      }
      sess.userId = userId;
      if (isSuperAdmin !== void 0) sess.isSuperAdmin = isSuperAdmin;
      try {
        const docMap = await firestore.collection("user_email_accounts").doc(userId).get();
        if (docMap.exists) {
          const data = docMap.data();
      ...
      if (accounts && Array.isArray(accounts) && sess.accounts.length === 0) {
        accounts.forEach((acc) => {
          if (!sess.accounts.find((a) => a.email === acc.email)) {
            sess.accounts.push({
              ...acc,
  ...
  app.post("/api/scheduler/confirm", async (req, res) => {
    ...
    if (!ownerSession || !ownerSession.accounts || ownerSession.accounts.length === 0) {
      await firestore.collection("mail_queue").add({
        ...
        to: guestEmail,
        bcc: ownerEmail,
        subject: `Confirmaci\xF3n de Reuni\xF3n: ${date} a las ${time}`,
        htmlBody: `
                <div style="font-family: sans-serif; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
                <h2 style="color: #6366f1;">\xA1Hola ${guestName}!</h2>
                <p>Tu reuni\xF3n ha sido agendada con \xE9xito.</p>
                <div style="background: #f9fafb; padding: 15px; border-radius: 8px; margin: 20px 0;">
                    <p><strong>Fecha:</strong> ${date}</p>
                    <p><strong>Hora:</strong> ${time}</p>
                    <p><strong>Notas:</strong> ${notes || "Sin notas adicionales"}</p>
                </div>
                <p>Para unirte a la reuni\xF3n en el momento acordado, haz clic en el siguiente enlace:</p>
                ...
                <p style="font-size: 12px; color: #aaa; text-align: center;">Este es un mensaje autom\xE1tico de Goatify Scheduler.</p>
                </div>
            `,
        status: "pending",
    ...
    const ownerMailOptions = {
      ...
      to: ownerEmail,
      bcc: acc.email,
      subject: `NUEVA REUNI\xD3N AGENDADA: ${guestName} - ${date}`,
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
          <h2 style="color: #6366f1;">\xA1Hola!</h2>
          <p>Alguien ha agendado una nueva reuni\xF3n contigo.</p>
          <div style="background: #f9fafb; padding: 15px; border-radius: 8px; margin: 20px 0;">
          ...
          <a href="${meetingLink}" style="display: inline-block; background: #6366f1; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">Ver Reuni\xF3n</a>
        </div>
      `
    };
    try {
  ...
  app.post("/api/project/invite", async (req, res) => {
    ...
    if (!ownerSession || !ownerSession.accounts || ownerSession.accounts.length === 0) {
      await firestore.collection("mail_queue").add({
        ...
        subject: `Invitaci\xF3n de Colaboraci\xF3n: ${projectName}`,
        htmlBody: `
                ...
                <div style="background-color: #111827; padding: 40px 30px; text-align: center; background-image: linear-gradient(135deg, #111827 0%, #1e3a8a 100%);">
                    <h1 style="color: #ffffff; margin: 0; font-size: 20px; font-weight: 900; letter-spacing: -0.01em; text-transform: uppercase;">Invitaci\xF3n a Proyecto</h1>
                    <p style="color: #94a3b8; margin: 8px 0 0 0; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.2em;">Colaboraci\xF3n en Goatify</p>
                </div>
                <div style="padding: 40px 30px; background-color: #ffffff;">
                    ...
                    <p style="font-size: 15px; color: #475569; margin-bottom: 30px;">Haz clic en el bot\xF3n de abajo para aceptar la invitaci\xF3n y acceder:</p>
                    ...
                    <p style="margin-top: 30px; font-size: 13px; color: #94a3b8; text-align: center; line-height: 1.5;">Si a\xFAn no tienes cuenta, reg\xEDstrate con este mismo correo y el acceso se activar\xE1 autom\xE1ticamente.</p>
                </div>
                <div style="padding: 25px; text-align: center; background-color: #f8fafc; border-top: 1px solid #e5e7eb;">
                    <p style="font-size: 12px; color: #94a3b8; margin: 0;">Enviado v\xEDa Goatify Productivity Suite.<br/>\xA9 2026 Goatify.</p>
                </div>
                </div>
            `,
        status: "pending",
    ...
    const transporter = nodemailer.createTransport(
      acc.provider === "google" ? { host: "smtp.gmail.com", port: 465, secure: true, auth: { type: "OAuth2", user: acc.email, clientId: GOOGLE_CLIENT_ID, clientSecret: GOOGLE_CLIENT_SECRET, refreshToken: acc.tokens.refresh_token, accessToken: acc.tokens.access_token } } : acc.provider === "microsoft" ? { host: "smtp.office365.com", port: 587, secure: false, auth: { type: "OAuth2", user: acc.email, accessToken: acc.tokens.access_token }, tls: { ciphers: "SSLv3" } } : { host: acc.provider === "gmail_manual" ? "smtp.gmail.com" : acc.provider === "outlook_manual" ? "smtp.office365.com" : "smtppro.zoho.com", port: acc.provider === "outlook_manual" ? 587 : 465, secure: acc.provider !== "outlook_manual", auth: { user: acc.email, pass: acc.password } }
    );
    const mailOptions = {
      from: `"${ownerName} v\xEDa Goatify Proyectos" <${acc.email}>`,
      to: guestEmail,
      bcc: acc.email,
      subject: `Invitaci\xF3n de Colaboraci\xF3n: ${projectName}`,
      html: `
      ...
      await firestore.collection("mail_queue").add({
        ...
        htmlBody: `
                ...
                <div style="background-color: #111827; padding: 40px 30px; text-align: center; background-image: linear-gradient(135deg, #111827 0%, #1e3a8a 100%);">
                    ...
                    <p style="color: #94a3b8; margin: 8px 0 0 0; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.2em;">Sistema de Gesti\xF3n Goatify</p>
                </div>
                <div style="padding: 40px 30px; background-color: #ffffff;">
                    <p style="font-size: 16px; color: #1e293b; line-height: 1.6;">Hola,</p>
                    ...
                    <p style="font-size: 15px; color: #475569; margin-bottom: 30px;">Puedes revisar los detalles, adjuntos y tiempos de entrega haciendo clic en el bot\xF3n de abajo:</p>
                    <div style="text-align: center;">
                ...
                <div style="padding: 25px; text-align: center; background-color: #f8fafc; border-top: 1px solid #e5e7eb;">
                    <p style="font-size: 12px; color: #94a3b8; margin: 0;">Enviado autom\xE1ticamente v\xEDa Goatify Productivity Suite.<br/>\xA9 2026 Goatify.</p>
                </div>
                </div>
            `,
        status: "pending",
  ...
  app.post("/api/wallet/receipt", async (req, res) => {
    ...
    const mailOptions = {
      from: `"${ownerName} v\xEDa Goatify Finance" <${acc.email}>`,
      to: recipientEmail,
      bcc: acc.email,
      subject: `Recibo de Transacci\xF3n Intis: ${txId}`,
      html: `
    <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; max-width: 650px; background-color: #f8fafc; padding: 20px; margin: 0 auto;">
        <div style="background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.05); border: 1px solid #e2e8f0;">
            <div style="background-color: #111827; padding: 40px 30px; text-align: center; background-image: linear-gradient(135deg, #111827 0%, #4c1d95 100%);">
                ...
                <p style="color: #94a3b8; margin: 8px 0 0 0; font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.2em;">Comprobante de Env\xEDo</p>
            </div>
            
            <div style="padding: 40px 30px;">
                <p style="font-size: 16px; color: #1e293b; margin-bottom: 25px;">\xA1Hola!</p>
                <p style="font-size: 15px; color: #475569; line-height: 1.6; margin-bottom: 30px;"><b>${ownerName}</b> te ha enviado Intis a trav\xE9s de la infraestructura financiera de Goatify. Aqu\xED est\xE1n los detalles:</p>
                ...
                <div style="background-color: #f8fafc; border-radius: 16px; border: 1px solid #e2e8f0; padding: 30px; margin-bottom: 35px;">
                    ...
                    <div style="border-top: 1px solid #e2e8f0; padding-top: 25px;">
                        <table style="width: 100%; border-collapse: collapse;">
                            <tr>
                                <td style="padding: 8px 0; color: #64748b; font-size: 13px; font-weight: 600; width: 40%;">ID de Transacci\xF3n:</td>
                ...
                <div style="text-align: center;">
                    <p style="font-size: 14px; color: #64748b; margin-bottom: 25px;">Puedes revisar tu billetera en la aplicaci\xF3n para ver tu nuevo saldo.</p>
            ...
            <div style="padding: 30px; text-align: center; background-color: #f8fafc; border-top: 1px solid #e2e8f0;">
                <p style="font-size: 12px; color: #94a3b8; margin: 0;">Enviado v\xEDa Goatify Productivity Suite.<br/>\xA9 2026 Goatify Innovation Lab.</p>
            </div>
        </div>
    </div>
      `
  ...
  app.post("/api/loyalty/registration", async (req, res) => {
    ...
    const mailOptions = {
      ...
      to: userEmail,
      bcc: acc.email,
      subject: `\xA1Te has unido al Programa de Fidelizaci\xF3n de ${projectName}!`,
      html: `
<!DOCTYPE html>
<html>
<head>
...
<div class="email-container" style="font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; width: 100%; max-width: 850px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06); border: 1px solid #e2e8f0; box-sizing: border-box;">
  ...
  <div style="padding: 40px 20px; background-color: #ffffff; box-sizing: border-box;">
    ...
    <p style="font-size: 16px; color: #334155; line-height: 1.8; margin-bottom: 30px;">
      Te has registrado exitosamente en nuestro programa de fidelizaci\xF3n. Estamos muy felices de tenerte con nosotros y valorar tu lealtad.
    </p>
    ...
    <p style="font-size: 16px; color: #334155; line-height: 1.8; margin-bottom: 35px;">
      Cada vez que nos visites, aseg\xFArate de registrar tu consumo para acumular puntos y acercarte a tu recompensa.
    </p>
  ...
  <div style="padding: 24px 20px; text-align: center; background-color: #f8fafc; border-top: 1px solid #e2e8f0; box-sizing: border-box;">
    <p style="font-size: 12px; color: #94a3b8; margin: 0; font-weight: 500;">\xA9 2026 ${projectName} - Gestionado con Goatify</p>
  </div>
</div>
</body>
</html>
  ...
  app.post("/api/loyalty/processed", async (req, res) => {
    ...
    const mailOptions = {
      ...
      to,
      subject: isApproved ? `\u2705 Consumo Aprobado en ${projectName}` : `\u274C Informaci\xF3n sobre tu registro en ${projectName}`,
      html: `
<!DOCTYPE html>
<html>
<head>
...
<div class="email-container" style="font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; width: 100%; max-width: 850px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06); border: 1px solid #e2e8f0; box-sizing: border-box;">
  ...
  <div style="background: linear-gradient(135deg, ${isApproved ? "#059669 0%, #047857 100%" : "#475569 0%, #334155 100%"}); padding: 40px 20px; text-align: center; box-sizing: border-box;">
    ...
    <h1 style="margin: 0; color: #ffffff; font-size: 28px; font-weight: 800; line-height: 1.3; letter-spacing: -0.5px;">
      ${isApproved ? "\xA1Visita Validada!" : "Registro Actualizado"}
    </h1>
  </div>
  ...
  <div style="padding: 40px 20px; background-color: #ffffff; box-sizing: border-box;">
    ...
    <p style="font-size: 16px; color: #334155; line-height: 1.8; margin-bottom: 30px;">
        ${isApproved ? `Tu visita en <strong>${projectName}</strong> ha sido aprobada. \xA1Est\xE1s un paso m\xE1s cerca de tu premio: ${rewardName}!` : `Lamentamos informarte que tu registro en <strong>${projectName}</strong> no ha podido ser validado en esta ocasi\xF3n.`}
    </p>
    ...
    <div style="background-color: #f0fdf4; border-left: 4px solid #10b981; padding: 24px 16px; border-radius: 0 8px 8px 0; margin-bottom: 35px; box-shadow: inset 0 2px 4px 0 rgba(0, 0, 0, 0.02); box-sizing: border-box;">
      ...
      <p style="margin: 0; color: #166534; font-size: 15px;">\xA1Sigue as\xED para completar tu tarjeta y disfrutar tu premio!</p>
    </div>
    ` : `
  ...
  app.post("/api/forms/notify", async (req, res) => {
    ...
    if (guestEmail) {
      guestMailOptions = {
        ...
        to: guestEmail,
        bcc: acc.email,
        subject: `Confirmaci\xF3n de env\xEDo: ${formName}`,
        html: `
              <div style="font-family: sans-serif; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
                <h2 style="color: #6366f1;">\xA1Gracias por contactarnos!</h2>
                ...
                <hr style="margin: 30px 0; border: none; border-top: 1px solid #eee;" />
                <p style="font-size: 12px; color: #aaa; text-align: center;">Enviado v\xEDa Goatify Productivity Suite.</p>
              </div>
            `
      };
    }
  ...
  app.get("/api/auth/google/callback", async (req, res) => {
    ...
    try {
      ...
      return res.status(400).send("Error: Estado de sesi\xF3n inv\xE1lido");
    }
    ...
    try {
      ...
      if (!session.accounts.find((a) => a.email.toLowerCase() === email)) {
        ...
        const acc = session.accounts.find((a) => a.email.toLowerCase() === email);
      ...
      res.status(500).send("Error en la autenticaci\xF3n con Google");
    }
  });
  app.get("/api/auth/microsoft/url", requireAuth, async (req, res) => {
    const redirectUri = req.query.redirectUri;
    const token = req.headers.authorization?.split(" ")[1] || req.query.token;
    const stateObj = { token, redirectUri };
    const stateStr = Buffer.from(JSON.stringify(stateObj)).toString("base64");
    const authCodeUrlParameters = {
      ...
      redirectUri,
      state: stateStr
      // Pass both token and redirectUri in state
    };
    try {
  ...
  app.get("/api/auth/microsoft/callback", async (req, res) => {
    ...
    if (!token) {
      return res.status(401).send("No autorizado: Falta token de sesi\xF3n");
    }
    if (!sessions.has(token)) {
      sessions.set(token, { id: token, accounts: [] });
    }
    ...
    try {
          ...
          <html>
            <body>
              <script>
                ...
                setTimeout(() => {
                  document.body.innerHTML = '<div style="font-family: sans-serif; text-align: center; padding: 40px;"><h2>\xA1Autenticaci\xF3n exitosa! \u{1F389}</h2><p>Ya puedes cerrar esta peque\xF1a ventana y volver a la aplicaci\xF3n principal.</p><button onclick="window.close()" style="padding: 10px 20px; background: #6366f1; color: white; border: none; border-radius: 8px; cursor: pointer;">Cerrar ventana</button></div>';
                }, 1000);
              </script>
              <p style="font-family: sans-serif; text-align: center; padding: 40px;">Autenticaci\xF3n exitosa. Cerrando ventana...</p>
            </body>
          </html>
        `);
    } catch (error) {
      ...
      res.status(500).send(`Error en la autenticaci\xF3n con Microsoft: ${error.message || "Error desconocido"}`);
    }
  });
  app.get("/api/accounts", requireAuth, (req, res) => {
    res.json({ accounts: req.session.accounts.map((a) => ({ id: a.id, email: a.email, provider: a.provider })) });
  });
  app.post("/api/accounts", requireAuth, async (req, res) => {
    const { email, password, provider: selectedProvider } = req.body;
    const emailLower = email.toLowerCase();
    if (req.session.accounts.find((a) => a.email.toLowerCase() === emailLower)) {
      return res.json({ success: true, accounts: req.session.accounts.map((a) => ({ id: a.id, email: a.email, provider: a.provider })) });
    }
    ...
    try {
      ...
      res.json({ success: true, message: "\xA1Cuenta conectada con \xE9xito!", accounts: req.session.accounts.map((a) => ({ id: a.id, email: a.email, provider: a.provider })) });
    } catch (err) {
      console.error("Error al conectar cuenta:", err);
      res.status(401).json({ error: `Error: Credenciales inv\xE1lidas. Verifica tu correo y contrase\xF1a.` });
    }
  });
  app.delete("/api/accounts/:id", requireAuth, async (req, res) => {
    const { id } = req.params;
    req.session.accounts = req.session.accounts.filter((a) => a.id !== id);
    if (req.session.userId) await syncAccountsToFirestore(req.session.userId, req.session.accounts);
    res.json({ success: true, accounts: req.session.accounts.map((a) => ({ id: a.id, email: a.email, provider: a.provider })) });
  });
  app.patch("/api/accounts/:id", requireAuth, async (req, res) => {
    const { id } = req.params;
    const { password } = req.body;
    const acc = req.session.accounts.find((a) => a.id === id);
    ...
    try {
      ...
      console.error("Error al validar nueva contrase\xF1a:", err);
      res.status(401).json({ error: "No se pudo validar la nueva contrase\xF1a. Verifica tus datos." });
    }
  });
  const getImapFolder = async (client, folderType) => {
    try {
  ...
  app.get("/api/emails", requireAuth, async (req, res) => {
    ...
    const allEmails = [];
    const limitCount = parseInt(limitQuery) || 50;
    const skip = parseInt(offset) || 0;
    const forceRefresh = refresh === "true";
    let accountsToFetch = accountId === "all" || !accountId ? req.session.accounts || [] : (req.session.accounts || []).filter((a) => a.id === accountId);
    ...
    for (const acc of accountsToFetch) {
      ...
      try {
        ...
        const folderToOpen = q ? "archive" : folder;
        ...
        try {
          ...
          if (paginatedUids.length > 0) {
            ...
            for (let msg of fetchedMsgs) {
              if (validAccEmailsCount >= limitCount) break;
              if (msg.flags && msg.flags.has("\\Deleted")) continue;
              validAccEmailsCount++;
              const parsed = await simpleParser(msg.source);
              const realDate = msg.envelope?.date || parsed.date || /* @__PURE__ */ new Date();
              allEmails.push({
                id: `${acc.id}-${msg.uid}`,
                accountId: acc.id,
                ...
                sender: {
                  name: parsed.from?.value?.[0]?.name || parsed.from?.value?.[0]?.address?.split("@")[0] || "Desconocido",
                  email: parsed.from?.value?.[0]?.address || ""
                },
                to: parsed.to?.value?.map((v) => ({ name: v.name || v.address?.split("@")[0], email: v.address })) || [],
  ...
  app.get("/api/emails/:id", requireAuth, async (req, res) => {
    ...
    const acc = req.session.accounts.find((a) => a.id === accountId);
    if (!acc) return res.status(404).json({ error: "Cuenta no encontrada" });
    await refreshAccountTokens(acc);
    const clientOptions = getImapClientOptions(acc);
    const client = new ImapFlow(clientOptions);
    try {
      await client.connect();
      const foldersToTry = folder ? [await getImapFolder(client, folder), "INBOX", "[Gmail]/Sent Mail", "Sent", "Drafts", "Trash", "Spam", "Archive"] : ["INBOX", "[Gmail]/Sent Mail", "Sent", "Drafts", "Trash", "Spam", "Archive"];
      let foundMsg = null;
  ...
  app.put("/api/emails/:id/move", requireAuth, async (req, res) => {
    const { id } = req.params;
  ...
  app.put("/api/emails/bulk-move", requireAuth, async (req, res) => {
  ...
  app.put("/api/emails/:id/read", requireAuth, async (req, res) => {
    const { id } = req.params;
    const { sourceFolder, read: read2 } = req.body;
    const lastDash = id.lastIndexOf("-");
    const accountId = id.substring(0, lastDash);
    const uid = id.substring(lastDash + 1);
    const acc = req.session.accounts.find((a) => a.id === accountId);
  ...
  app.post("/api/send", requireAuth, async (req, res) => {
    ...
    const formatEmails = (str) => {
      if (!str) return "";
      return str.replace(/[;\s]+/g, ",").split(",").map((e) => e.trim()).filter((e) => e).join(", ");
    };
    const isGoogleOAuth = acc.provider === "google";
    const isMicrosoftOAuth = acc.provider === "microsoft";
    const isGmailManual = acc.provider === "gmail_manual";
    ...
    try {
      try {
        ...
        return res.status(500).json({ error: "Error de conexi\xF3n con el servidor de correo. Revisa tus credenciales." });
      }
      const mailOptions = {
      ...
      if (!isGoogleOAuth && !isGmailManual) {
        try {
          const clientOptions = getImapClientOptions(acc);
          const client = new ImapFlow(clientOptions);
          await client.connect();
          const sentFolder = await getImapFolder(client, "sent");
          const dateHeader = `Date: ${(/* @__PURE__ */ new Date()).toUTCString()}\r
`;
          const toHeader = mailOptions.to ? `To: ${mailOptions.to}\r
` : "";
          const ccHeader = mailOptions.cc ? `Cc: ${mailOptions.cc}\r
` : "";
          const bccHeader = mailOptions.bcc ? `Bcc: ${mailOptions.bcc}\r
` : "";
          let rawMessage = `${dateHeader}From: ${mailOptions.from}\r
${toHeader}${ccHeader}${bccHeader}Subject: ${subject || "Sin asunto"}\r
MIME-Version: 1.0\r
...
${body || ""}`;
  ...
  app.post("/api/drafts", requireAuth, async (req, res) => {
    ...
    try {
      ...
      const dateHeader = `Date: ${(/* @__PURE__ */ new Date()).toUTCString()}\r
`;
      const rawMessage = `${dateHeader}From: ${acc.email}\r
To: ${formatEmails(to)}\r
Cc: ${formatEmails(cc)}\r
Bcc: ${formatEmails(bcc)}\r
Subject: ${subject || "Borrador sin asunto"}\r
MIME-Version: 1.0\r
Content-Type: text/html; charset=utf-8\r
\r
${body || ""}`;
  ...
  app.post("/api/files/extract", requireFirebaseUser, async (req, res) => {
...
`;
        ...
        try {
          ...
          extractedText = "Error al extraer texto del PDF en servidor. Gemini intentar\xE1 procesarlo como imagen/PDF nativo.";
        }
...
`;
        ...
        extractedText = `Contenido binario no extra\xEDble directamente para ${fileName}. Se enviar\xE1 metadata b\xE1sica.`;
      }
      res.json({
        ok: true,
        fileName,
        extractedText: extractedText.substring(0, 15e4),
        // Límite de seguridad
  ...
  app.post("/api/artifacts/generate", requireFirebaseUser, async (req, res) => {
    ...
    try {
      ...
      const generatePdfBuffer = async (docContent) => {
        ...
        const wrapText = (text, maxWidth, font2, size) => {
          const words = text.split(" ");
          const lines = [];
          let currentLine = "";
          for (const word of words) {
            const testLine = currentLine ? `${currentLine} ${word}` : word;
            const width = font2.widthOfTextAtSize(testLine, size);
            if (width <= maxWidth) {
        ...
        for (let i = 0; i < totalPages; i++) {
          ...
          p.drawText(`P\xE1gina ${i + 1} de ${totalPages} | Goatify Docs`, {
            x: 50,
            y: 30,
            size: 8,
            font,
      ...
      const generateDocxBuffer = async (docContent) => {
      ...
      const generateXlsxBuffer = async (docContent, sData) => {
      ...
      if (saveToDrive) {
        ...
        try {
          ...
          await driveRef.set({ personalFiles: updatedFiles, updatedAt: (/* @__PURE__ */ new Date()).toISOString() }, { merge: true });
          driveSaved = true;
  ...
  app.post("/api/tasks/create", requireFirebaseUser, async (req, res) => {
    ...
    try {
      const taskObj = {
        ...
        date: dueDate?.split("T")[0] || (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
  ...
  app.post("/api/projects/create", requireFirebaseUser, async (req, res) => {
  ...
  app.post("/api/calendar/events/create", requireFirebaseUser, async (req, res) => {
    ...
    try {
      ...
      const eventObj = {
        ...
        startAt: startDate,
        endAt: endDate || new Date(new Date(startDate).getTime() + 30 * 6e4).toISOString(),
        endDate: endDate || new Date(new Date(startDate).getTime() + 30 * 6e4).toISOString(),
        participants: [userId],
        invited: attendees || [],
        caller: {
          uid: userId,
  ...
  app.post("/api/gemini/chat", requireFirebaseUser, async (req, res) => {
  ...
  app.post("/api/gemini/tts", requireFirebaseUser, async (req, res) => {
    ...
    try {
      ...
      await recordServerUsageTelemetry(req.user.uid, { module: "voice", model: "gemini-3.1-flash-tts-preview", usageMetadata: response.usageMetadata, requestId, fallbackCost: 2e-3, action: "tts" });
  ...
  app.post("/api/debug/tts", requireFirebaseUser, async (req, res) => {
  ...
  app.post("/api/gemini/images", requireFirebaseUser, async (req, res) => {
    ...
    try {
      ...
      if (!base64Bytes) {
        ...
        return res.status(502).json({
          error: "No se recibi\xF3 imagen desde Gemini Imagen.",
          model: imagenModel
        });
      }
      return res.json({
  ...
  app.post("/api/ai/chat", requireFirebaseUser, async (req, res) => {
    const { history, systemInstruction, model, config, module: reqModule, requestId } = req.body;
    const { key: apiKey } = getValidGeminiApiKey();
    const creditAmount = serverModuleChatCost(reqModule);
    let creditConsumed = false;
  ...
  app.post("/api/gemini/stream", requireFirebaseUser, async (req, res) => {
    ...
    try {
      ...
      return res.status(e?.status || 402).json({ ok: false, error: e?.message || "L\xEDmite del plan alcanzado.", code: "PLAN_LIMIT_REACHED" });
    }
...
`);
  ...
  app.post("/api/gemini/media", requireFirebaseUser, async (req, res) => {
    const { prompt, type, fileData, config, requestId } = req.body;
    if (type === "image-gen") {
      return res.status(400).json({ error: "Para generar im\xE1genes usa el endpoint /api/gemini/images" });
    }
    const { key: apiKey } = getValidGeminiApiKey();
    const mediaFeature = String(fileData?.mimeType || "").startsWith("video/") ? "ai_video" : "ai_chat";
  ...
  liveWss.on("connection", async (clientWs, req) => {
                  ...
                  functionDeclarations: [
                    {
                      ...
                      parameters: {
                        type: "object",
                        properties: {
                          title: { type: "string", description: "T\xEDtulo de la tarea" },
                          description: { type: "string", description: "Descripci\xF3n detallada" },
                    ...
                    {
                      name: "create_project",
                      description: "Inicia un nuevo proyecto estrat\xE9gico.",
                      parameters: {
                        type: "object",
                        properties: {
                    ...
                    {
                      name: "create_event",
                      description: "Agenda un evento o reuni\xF3n en el calendario.",
                      parameters: {
                        type: "object",
                        properties: {
                          title: { type: "string", description: "T\xEDtulo del evento" },
                          description: { type: "string", description: "Descripci\xF3n" },
                    ...
                    {
                      name: "send_email",
                      description: "Env\xEDa un correo electr\xF3nico a un destinatario.",
                      parameters: {
                        type: "object",
                        properties: {
                    ...
                    },
                    {
                      ...
                      description: "Busca informaci\xF3n actualizada en internet sobre cualquier tema.",
                      parameters: {
                        type: "object",
                        properties: {
                          query: { type: "string", description: "B\xFAsqueda a realizar" }
                        },
                        required: ["query"]
                      }
                    }
